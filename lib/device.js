...
function Device(brokerUrl, options) {
    ...
    var deviceOptions = __.pick(options, "onControl", "onConfig", "initialLoad", "about");
...
Device.prototype.loadConf = function(callback) {
    ...
    _private.client.publish('$/devmgmt', JSON.stringify(request));
};
...
Device.prototype.unsubscribe = function( topic, callback ) {
    this._validateTopic(topic);
    var _private = $(this);
    _private.client.unsubscribe( topic, callback );
};

Device.prototype.close = function( force, callback ) {
    var _private = $(this);
    var client = _private.client;
    client.publish("$/devmgmt", JSON.stringify({name: "DeviceUnregist"}), {qos: 1});
...
Device.prototype._regist = function() {
    ...
    var that = this;
    var client = _private.client;
    client.publish("$/devmgmt",
        JSON.stringify({name: "DeviceRegist", data: __.pick(_private.about, "name", "desc")}),
        ...
        if (error) {
            ...
            if (_private.onControl || _private.onConfig) {
                ...
                client.subscribe("$/device", {qos: 2}, function(error) {
                    if (error) {
...
Device.prototype._handleManagement = function(topic, payload) {
    ...
    function makeResponse(result) {
        ...
        if (!utils.isUndefined(ack)) {
            var responseTopic = "$/server/"+message.from;
            var client = _private.client;
