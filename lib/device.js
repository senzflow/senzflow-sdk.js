...
Device.prototype.nodeOnline = function(identity, opts) {
    ...
    var that = this;
    _private.client.publish('$DM', request, {qos: 0}, function(error, ok) {
        if (error) {
...
Device.prototype.close = function( graceful, callback ) {
    ...
    if (graceful) {
        client.publish("$DM", JSON.stringify({type: "Off"}), {qos: 1}, function() {});
    }
...
function identityValidate(identity) {
    if (!/^\.?[\w\d_-]+$/.test(identity)) {
        throw new Error('Only alpha-num-low-dash-hyphen are allowed for identity \''+identity+'\'.');
    }
}
function debug() {
...
function connect(deviceID, connectOptions) {
    ...
    return mqtt.connect(__.extend(connectOptions, {will: {
        topic: "$DM",
        payload: JSON.stringify({type: "Off"}),
        qos: 1,
        retain: false
    }}));
}
